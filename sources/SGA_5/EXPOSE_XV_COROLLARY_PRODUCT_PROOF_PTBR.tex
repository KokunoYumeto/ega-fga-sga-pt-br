...
Denotando estes endomorfismos por $\varphi'$ e $\varphi$, mostremos, por exemplo, que $\varphi\circ\varphi'=\id$; isso resulta do seguinte diagrama comutativo:
\[
\resizebox{\textwidth}{!}{%
...
\]
que é comutativo devido a
\[
\fr_{X\times X'}=(\fr_X\times\id_{X'})\circ(\id_X\times\fr_{X'})
\]
para o triângulo superior e de
\[
\Fr^*_{F\boxt F'/X\times X'}=(\Fr^*_{F/X}\otimes\id_{F'})\circ(\id_{\fr_X^*F}\otimes\Fr^*_{F'/X'})
\]
para o triângulo inferior, e no qual o composto das duas flechas oblíquas é a identidade segundo a proposição 2c) (n\textsuperscript{o}2). Por meio de um diagrama análogo que troca os papéis de $(X,F)$ e $(X',F')$, obtém-se $\varphi'\circ\varphi=\id$.
